...
public class NVTrackerEvent extends Fragment {
	...
	public void onActivityCreated(Bundle savedInstanceState) {
		...
		NVTrackerEventApi eventApi = NVTrackerAPIFactory.getEventAPI(getApplicationContext());

		// The following provides three ways of tracking events:
		//	- single event message.
		//  - single event message object 
		//  - a list event message objects. 
		//
		...
		NVTrackerLogRecord logRecord = new NVTrackerLogRecord();
		logRecord.setEvent_type("test02");
		logRecord.setDetails("event details goes here");
		eventApi.logEvent(logRecord);

		// ---
		// Log a list of event objects
		List<NVTrackerLogRecord> logRecords = new ArrayList<NVTrackerLogRecord>();

		// Create two log record objects 
		for (int i=0; i< 2; i++) {
			NVTrackerLogRecord logRecord = new NVTrackerLogRecord();
			logRecord.setEvent_type("test"+i);
			logRecord.setDetails("event details goes here");
